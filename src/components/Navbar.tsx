  ...
  return (
    <nav 
      className={`fixed w-full z-50 transition-all duration-300 ${
        scrolled ? 'glass-dark py-2 shadow-lg' : 'bg-vme-dark/80 backdrop-blur-sm py-4 border-b border-white/5'
      }`}
    >
      ...
      <div className={`hidden md:block border-b border-white/10 pb-2 mb-2 transition-opacity duration-300 ${scrolled ? 'opacity-0 h-0 overflow-hidden' : 'opacity-100'}`}>
        <div className="container mx-auto px-6 flex justify-between items-center text-xs text-vme-blue uppercase tracking-widest font-medium">
          <div className="flex gap-6">
            ...
            <span className="flex items-center gap-2"><Mail size={14} className="text-vme-red" /> viveganme@gmail.com</span>
          </div>
          <div>Established 2021 | Precision Excellence</div>
        </div>
      </div>

      <div className="container mx-auto px-6 flex justify-between items-center">
        <Link href="/" className="flex items-center gap-3 group">
          <div className="relative w-12 h-12 bg-white rounded-lg p-1 group-hover:scale-105 transition-transform">
            <Image 
              src="/logos/vme-logo.png" 
              alt="VME Logo" 
              fill 
